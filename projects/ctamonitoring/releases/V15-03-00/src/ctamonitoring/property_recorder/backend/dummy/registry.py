...
@change: $LastChangedDate: 2013-09-24 22:53:25 +0200 (Tue, 24 Sep 2013) $
...
class Registry(object):
    ...
    def register(self,
        ...
        @param property_type_desc: Some property types need additional information
        to use the property monitoring data later. An enumeration is an example.
        Enumeration monitoring data is an integer. However, to relate these values
        to a "tag" one should provide a dictionary "tags -> values" here.
        Optional(None).
        @type property_type_desc: typically dict
        @param disable: The property recorder might detect a property within the
        system, but decide not to monitor it. It might still log the detection
        via this call but "disable" the buffer that is returned.
        Optional(False).
